...
RUN pip install --no-cache-dir --upgrade pip setuptools wheel && \
    pip install --no-cache-dir torch==2.7.0+cu128 torchvision==0.22.0+cu128 'numpy<2.0.0' --extra-index-url https://download.pytorch.org/whl/cu128 && \
...
RUN export TORCH_CUDA_ARCH_LIST="$(echo "$CUDA_ARCHITECTURES" | tr ';' '\n' | awk '{n=length($0); if(n==3) print substr($0,1,2)"."substr($0,3); else if($0+0>70) print substr($0,1,1)"."substr($0,2)}' | tr '\n' ' ' | sed 's/ $//')" && \
    export MAX_JOBS=4 && \
